---
type: area
status: "In Progress"
importance: 3
urgency: 1
progress: 25
area: ""
review_date: 2026-07-20
tags: [relationships]
---

# Relationships

**Responsibilities** (ongoing, never "done"):
- Nurture key personal relationships and community ties.
- Contribute to and draw from networks (online/offline) aligned with values.

**Standards / Metrics** (how I know the area is healthy):
- Regular meaningful contact
- Reciprocal support visible
- Low loneliness + high signal connections

**Current Focus / Active Projects**:
- (leverage collab-finder and presence projects for professional community)

## Review Log
- 2026-07-06: Created from schema as canonical flat area note.
