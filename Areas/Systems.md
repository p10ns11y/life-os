---
type: area
status: "In Progress"
importance: 4
urgency: 2
progress: 68
area: ""
review_date: 2026-09-07
tags: [automation, systems]
---

# Systems

**Responsibilities** (ongoing, never "done"):
- Personal environment, automation, self-hosted infra, daily operating systems (shellyxz, arch-machine, etc.).
- Reliability, backups, low-friction tooling for living and working.

**Standards / Metrics** (how I know the area is healthy):
- Systems that "just work" and self-audit
- Minimal toil; high uptime for critical services
- Documented boundaries and quick recovery

**Current Focus / Active Projects**:
- **Plate bots:** Fleet Desk (ensembly fleet; crew HOOTL) · Steward (packedbox/voice/kernel) · Chief of Staff (token budget) — roles on [[UI/Dashboard]]
- [[Projects/packedbox/README|packedbox]] — Phase 4 NavigationView merged; Steward ops
- [[Projects/skills/README|skills]] — control-feeder + grok-host-prep (08-14); park new authoring
- [[Projects/arch-machine/README|arch-machine]] — quiet week; return to `sentinel` (behind 1); untracked `.agents/` copies are noise
- [[Projects/ensembly/README|ensembly]] — crew HOOTL; operator arch check-ins only; Fleet Desk + Steward keep shipping
- [[Projects/shellyxz/README|shellyxz]] · [[Projects/premflow/README|premflow]] · [[Projects/peram-vault/README|peram-vault]] — maintenance

## Review Log
- 2026-09-07: Operator plate — Fleet Desk + Steward roles; packedbox Phase 4; ensembly crew HOOTL (not a fifth slot). [[Reviews/plate-2026-09-07]].
- 2026-08-15: Weekly review — grok-build/skills/plugins moved; arch-machine no product commits. Dirty: wealth-core (Finance-adjacent), not Systems kernel.
- 2026-08-10: Weekly review — systems park for W33; harness/skills shipped prior days; finance + career take capacity.
- 2026-07-21: arch-machine week — keeper (#28 + open #33 PATH/rebind/loop), archy #29, groxy #31–#32; eye-comfort monitor-only; avante Grok ACP stays host nvim config.
- 2026-07-13: Linked peram-vault (PQ encrypted local SoT; Collections export satellite).
- 2026-07-06: Created from schema as canonical flat area note (originally "Home", renamed to Systems for clarity).
